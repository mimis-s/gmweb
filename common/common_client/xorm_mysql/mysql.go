...
import (
	...
	"xorm.io/xorm/core"
	xorm_dialects "xorm.io/xorm/dialects"
...
func (db *XormMysql) Init(uri *xorm_dialects.URI) error {
...
func (db *XormMysql) Alias(col string) string {
...
func (db *XormMysql) Version(ctx context.Context, queryer core.Queryer) (*schemas.Version, error) {
...
func (db *XormMysql) Features() *xorm_dialects.DialectFeatures {
	return &xorm_dialects.DialectFeatures{
		AutoincrMode: IncrAutoincrMode,
	}
}

func (db *XormMysql) SetParams(params map[string]string) {
...
func (db *XormMysql) SQLType(c *schemas.Column) string {
	var res string
	var isUnsigned bool
...
func (db *XormMysql) ColumnTypeKind(t string) int {
...
func (db *XormMysql) IsReserved(name string) bool {
	_, ok := mysqlReservedWords[strings.ToUpper(name)]
	return ok
}

func (db *XormMysql) AutoIncrStr() string {
	return "AUTO_INCREMENT"
}

func (db *XormMysql) IndexCheckSQL(tableName, idxName string) (string, []interface{}) {
...
func (db *XormMysql) IsTableExist(queryer core.Queryer, ctx context.Context, tableName string) (bool, error) {
	sql := "SELECT `TABLE_NAME` from `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA`=? and `TABLE_NAME`=?"
	return db.HasRecords(queryer, ctx, sql, db.uri.DBName, tableName)
}

func (db *XormMysql) AddColumnSQL(tableName string, col *schemas.Column) string {
...
func (db *XormMysql) ModifyColumnSQL(tableName string, col *schemas.Column) string {
...
func (db *XormMysql) GetColumns(queryer core.Queryer, ctx context.Context, tableName string) ([]string, map[string]*schemas.Column, error) {
...
func (db *XormMysql) GetTables(queryer core.Queryer, ctx context.Context) ([]*schemas.Table, error) {
...
func (db *XormMysql) SetQuotePolicy(quotePolicy xorm_dialects.QuotePolicy) {
	switch quotePolicy {
	case xorm_dialects.QuotePolicyNone:
		q := mysqlQuoter
		q.IsReserved = schemas.AlwaysNoReserve
		db.quoter = q
	case xorm_dialects.QuotePolicyReserved:
		q := mysqlQuoter
		q.IsReserved = db.IsReserved
		db.quoter = q
	case xorm_dialects.QuotePolicyAlways:
		fallthrough
	default:
		db.quoter = mysqlQuoter
	}
...
func (db *XormMysql) GetIndexes(queryer core.Queryer, ctx context.Context, tableName string) (map[string]*schemas.Index, error) {
	args := []interface{}{db.uri.DBName, tableName}
...
func (db *XormMysql) CreateTableSQL(ctx context.Context, queryer core.Queryer, table *schemas.Table, tableName string) (string, bool, error) {
	if tableName == "" {
...
func (db *XormMysql) Filters() []xorm_dialects.Filter {
	return []xorm_dialects.Filter{}
}
