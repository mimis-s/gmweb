...
type Dialect interface {
	Init(*xorm_dialects.URI) error
	URI() *xorm_dialects.URI
	Version(ctx context.Context, queryer core.Queryer) (*schemas.Version, error)
	Features() *xorm_dialects.DialectFeatures
	...
	SetQuotePolicy(quotePolicy xorm_dialects.QuotePolicy)
...
func (db *Base) Init(dialect Dialect, uri *xorm_dialects.URI) error {
	db.dialect, db.uri = dialect, uri
	return nil
}

// URI returns the uri of database
func (db *Base) URI() *xorm_dialects.URI {
	return db.uri
}
